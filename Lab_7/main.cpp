...
template<typename T>
class NewSet : public Set<T>{
private:
    template <class V>
    friend void decision1(NewSet<V> &, NewSet<V> &, NewSet<V> &);
    template <class V>
    friend void decision2(NewSet<V> &, NewSet<V> &, NewSet<V> &);
public:
    NewSet(int size) : Set<T>(size){}
    NewSet() : Set<T>(){}
};

template<typename T>
void decision1(NewSet<T> &fs, NewSet<T> &ss, NewSet<T> &ts){
    Set<string> unificationSet = fs + ss + ts;
    cout << unificationSet;
}

template<typename T>
void decision2(NewSet<T> &fs, NewSet<T> &ss, NewSet<T> &ts){
